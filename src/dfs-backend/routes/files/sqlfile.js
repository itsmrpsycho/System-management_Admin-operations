...
router.post('/api/upload-file-path', (req, res) => {
  const { filePath } = req.body;

  // Validate and sanitize the filePath here

  // Example: Read the file from the filePath
  const fileContent = fs.readFileSync(path.resolve(filePath), 'utf8');

  // Process the file as needed

  res.json({ message: 'File processed', uploadedFiles: [filePath] });
});
