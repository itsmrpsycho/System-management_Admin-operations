...
export default function AddDataModel() {
  ...
  // const [verisonData, setVersionData] = useState(null);
  ...
  useEffect(() => {
    try {
      ...
      // Handle the error (e.g., redirect, show an error message)
    }
  }, [data]);
  ...
  useEffect(() => {
    if (dummyData) {
      // setVersionData(dummyData.version || null);
      setDatasetId(dummyData.DatasetID || "");
      setmodelName(dummyData.Modelname || "");
      setGroupName(dummyData.Groupname || "");
      setDatasetVersionId(dummyData.DatasetVersionID || "");
      setGroupId(dummyData.GroupID || "")
      setauthorName(dummyData.AuthorName || "")
